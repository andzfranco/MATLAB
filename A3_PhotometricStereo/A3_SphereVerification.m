...
[X, Y] = meshgrid(1:size(n_f, 2), 1:size(n_f, 1)); % Create a grid of X and Y coordinates
Z = -n_f; % Depth (Z) values from the reconstruction
%Z = Z(0==sum(isnan(Z), 2));
%[X, Y] = meshgrid(1:size(Z, 2), 1:size(Z, 1)); % Create a grid of X and Y coordinates
%disp(Z)
%disp(X);
% Compute the center of the grid
centerX = size(n_f, 2) / 2;
centerY = size(n_f, 1) / 2;
%disp(centerX);
%disp(centerY);

% Calculate radial distances
distances = sqrt((X - centerX).^2 + (Y - centerY).^2 + Z.^2);
% Compute distances
distances = sqrt((X - centerX).^2 + (Y - centerY).^2 + Z.^2);
...
fprintf('Mean radial distance: %.2f\n', mean_distance);
fprintf('Standard deviation of radial distances: %.2f\n', std_distance);
